...
contract RxNftMinter is ERC721Base {
    ...
    //     require(
    //         has(pharmacies, msg.sender) == true,
    //         "Must have Pharmacy role."
    //     );
    //     _;
    // }
    ...
    // //Add Address to each role:
    // function addPatient(address _newPatient) external onlyDoctor onlyAdmin {
    //     add(patients, _newPatient);
    // }

    // function addPharmacy(
    //     address _newPharmacy
    // ) external onlyPatient onlyDoctor onlyAdmin {
    ...
    // // Remove Roles
    // function removePatient(address _oldPatient) external onlyDoctor onlyAdmin {
    //     remove(patients, _oldPatient);
    // }

    // function removePharmacy(
    //     address _oldPharmacy
    // ) external onlyPatient onlyDoctor onlyAdmin {
    ...
    function updateScript(uint256 tokenId, uint256 pillsFilled) public returns (uint256) {
        Script storage rx = allScripts[tokenId];
    ...
    function updateScriptExplicit(uint256 tokenId, uint256 pillsFilled) public {
        Script storage rx = allScripts[tokenId];
        require(pillsFilled >= rx.quantity, "Pills Filled must be less than or equal to Pills Prescribed");
