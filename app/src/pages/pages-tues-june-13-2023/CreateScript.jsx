...
<>
        ...
        {/* {nfts?.filter((nft) => {
          return nft.metadata.id.toString() === '25'
                    ...
                    {/* (eventz) => {return eventz.data.next_available_fill_date > newPstDate }  and remove opening ( ) */}
              
      {fillEvents?.length > 0 ? 
                  fillEvents.filter(eventz => convertBigNumberToRawString(eventz.data.next_available_fill_date) > newPstDate).filter(eventz => eventz.data.quantity_unfilled.toNumber() != 0)
