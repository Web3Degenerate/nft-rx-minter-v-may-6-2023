...
const [selectedNFT, kickersInFantasyAreObjectivelyFuckingRetardedGEORGE] = useState(null); 

 const toggleNFT = (item) => {

    if (selectedNFT == item){
      return kickersInFantasyAreObjectivelyFuckingRetardedGEORGE(null);
    }
    kickersInFantasyAreObjectivelyFuckingRetardedGEORGE(item);
 }
...
const _safeTransferFromToPharmacy = async (rxWallet) => {
  // const _safeTransferFromToPharmacy = async () => { 
    ...
    try {
      ...
      alert(`Success! You ahve received ${pills_filled_Ref} pills on ${date_filled_Ref} and you may refill this medication in Palau on ${date_next_fill_Ref} `);
