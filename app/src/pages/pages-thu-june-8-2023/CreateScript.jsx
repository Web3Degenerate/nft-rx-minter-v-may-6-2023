...
// Original idea for useRef came from: https://dev.to/kevinkh89/how-to-solve-input-delay-lagging-in-react-j2o
  ...
  import { addyShortner, formatDateTwoDigitYear, dayCalculator } from '../utils'
...
// Set initial "Next Fill Date"
    const nextFillDate = dayCalculator(date_prescribed_Ref, numberOfDays);
...
//v12.7 version: Added two additional parameters, date_prescribed_Ref formatted and perDiemMax in its own slot.
    // const data = await _createScript({ args: [wallet_address, wallet_address, name, description_Ref, medication_Ref, dob, 
    //   pt_physical_address, quantity_Ref, doctor, dea, date_prescribed_Ref, formatDateTwoDigitYear(date_prescribed_Ref), new Date(date_prescribed_Ref).getTime(), 
    //   perDiemMax, nextFillDate, formatDateTwoDigitYear(nextFillDate)] })
...
<>
        ...
        <h3>Potential Conflicts For Patient: {name} as of {newPstDate}</h3>
      ...
      {fillEvents?.length > 0 ? 
                  fillEvents.filter((eventz) => {
                      return eventz.data.next_available_fill_date > newPstDate
                    }
                        ...
                        <tr key={`${index}`}>
                            ...
                            <td>{eventz.data.next_available_fill_date}</td>
                        ...
                        <tr>
                              <td colSpan="9" className="text-center">
                                  <h5 style={{color:"white"}}>No Active Script Conflicts Found For {name}</h5>
                              </td>
                        </tr>
                    )}
...
{/* External Linking Solution From: https://herewecode.io/blog/react-router-link-to-external-url/ */}
    ...
    <div className="box_size">
                    ...
                    <div className="row">
                            <div className="col-md-2">Wallet Address:</div>
                    ...
                    <div className="row">
                            <div className="col-md-2">Physical Address:</div>
                            <div className="col-md-10">
                                {/* <input type="text" name="name" className="form-control" value={name} onChange={(e) => handleChange(e)} /> */}
                                {/* <input type="text" name="wallet_address" className="form-control" onChange={(e) => handleChange(e)} />  */}
                                {/* <input type="text" name="wallet_address" className="form-control" value={form.wallet_address} onChange={(e) => handleFormFieldChange('wallet_address', e)} />  */}
