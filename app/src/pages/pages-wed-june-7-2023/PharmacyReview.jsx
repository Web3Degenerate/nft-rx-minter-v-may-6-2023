...
//********************************************************* */
    ...
    const [updatedNextFillDate, setUpdatedNextFillDate] = useState('N/A');

    const maxPillsPerFuckingDay = nftz?.metadata.attributes[10].value;
...
const handleSubmitTransferToPatient = async (e) => { 
  ...
  const dynamicNextFillRef = useRef('N/A')
  ...
  const updateNextGodDamnFuckingFillDay = () => {
    // e.preventDefault();
    ...
    // const nextGodDamnFillDate = await calcNextFillDate();
        ...
        let quantity_filled_int = parseInt(getFuckingQuantityFilled)
        ...
        let get_Quantity_int = parseInt(getQuantgetDateOringinallyFuckingFilledity)
  
        if(get_Quantity_int - totalPillsFilled == 0){
            if(nftz?.metadata.attributes[6].value == 'N/A'){
              let nextGdFillDate = getNextFillDateCompleteFirstFill(inputDateFilled.current.value, pills_filled_Ref)
            }else{
               getNextFillDateCompleteSecondFill(inputDateFilled.current.value, pills_filled_Ref)
            }
        }else{
          if(nftz?.metadata.attributes[6].value == 'N/A'){
             getNextFillDatePartialFirstFill(inputDateFilled.current.value, pills_filled_Ref)
          }else{
            const nextGdFillDate = getNextFillDatePartialSecondFill(inputDateFilled.current.value, pills_filled_Ref)
            setUpdatedNextFillDate(nextGdFillDate)
        
          }
        }

        // handleSubmitUpdateFilled(e)
  }
...
// SET SCRIPT STATUS: 
 ...
 //v12.7 version - added formatted dates, updatedNextFillDate apart from filld ate and script_status (pending or complete)        
            ...
            //   formatDateTwoDigitYear(date_filled_Ref), updatedNextFillDate, formatDateTwoDigitYear(updatedNextFillDate), pharmacy_name, updateScriptStatus]);
